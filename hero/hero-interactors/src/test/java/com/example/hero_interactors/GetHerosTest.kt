...
class GetHerosTest {
    ...
    fun getHeros_success() = runBlocking {
        // setup could be done in another separate fun
        val heroDatabase = HeroDatabaseFake()
        val heroCache = HeroCacheFake(heroDatabase)
        val heroService = HeroServiceFake.build(
            type = HeroServiceResponseType.GoodData // good data expected
        )

        // we create our use case with the needed dependencies
        getHeros = GetHeros(
            cache = heroCache,
            service = heroService
        )
        ...
        // Execute the use-case and save the emissions on a list
